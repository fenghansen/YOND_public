...
class BaseParser():
    ...
    def parse(self):
        self.parser.add_argument('--runfile', '-f', default="runfiles/Gaussian/EstNet_k19.yml", type=Path, help="path to config")
...
class Base_Trainer():
    ...
    def get_lr_lambda_func(self):
        ...
        coldstart = True if 'coldstart' not in self.hyper else self.hyper['coldstart']
        if 'cos' in self.hyper['lr_scheduler'].lower():
            self.lr_lambda = lambda x: get_cos_lr(x, period=num_of_epochs//T, 
                lr=self.hyper['learning_rate'], peak=step_size, coldstart=coldstart) 
    ...
    def print_model_log(self):
        ...
        if torch.cuda.device_count() > 1:
            ...
            print(torch.distributed.is_available(), self.local_rank, self.world_size)
            torch.distributed.init_process_group(backend='nccl', rank=self.local_rank, world_size=self.world_size)
...
def get_cos_lr(step, period=1000, peak=20, lr=1e-4, ratio=0.4, coldstart=False):
    T = step // period
    decay = 2 ** T
    step = step % period
    if step <= peak and (not coldstart or T>0):
        mul = step / peak
    else:
